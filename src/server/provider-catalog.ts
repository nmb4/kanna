...
export const SERVER_PROVIDERS: ProviderCatalogEntry[] = PROVIDERS.map(
  (provider) =>
    provider.id === "codex"
      ? {
          ...provider,
...
export function normalizePiModelOptions(
  ...
  const thinkingLevel = modelOptions?.pi?.thinkingLevel;
  return {
    thinkingLevel: isPiReasoningEffort(thinkingLevel)
      ? thinkingLevel
