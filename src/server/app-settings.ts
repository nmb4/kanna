...
interface AppSettingsFile {
  ...
  providerDefaults?: {
    claude?: Partial<ProviderPreference<Partial<ClaudeModelOptions>>> & {
      effort?: unknown;
    };
    codex?: Partial<ProviderPreference<Partial<CodexModelOptions>>> & {
      effort?: unknown;
    };
    pi?: Partial<ProviderPreference<Partial<PiModelOptions>>>;
  };
}
...
function normalizeClaudePreference(value?: {
  ...
  const model = normalizeClaudeModelId(
  ...
  return {
    ...
    modelOptions: {
      reasoningEffort:
        !supportsClaudeMaxReasoningEffort(model) && normalizedEffort === "max"
          ? "high"
          : normalizedEffort,
      contextWindow: normalizeClaudeContextWindow(
        model,
        value?.modelOptions?.contextWindow,
      ),
    },
...
function normalizeCodexPreference(value?: {
  ...
  return {
    ...
    modelOptions: {
      ...
      fastMode:
        typeof value?.modelOptions?.fastMode === "boolean"
...
function normalizePiPreference(value?: {
  ...
  return {
    model: normalizePiModelId(
      typeof value?.model === "string" ? value.model : undefined,
    ),
    modelOptions: {
      thinkingLevel: isPiReasoningEffort(value?.modelOptions?.thinkingLevel)
        ? value.modelOptions.thinkingLevel
        : DEFAULT_PI_MODEL_OPTIONS.thinkingLevel,
    },
    planMode: value?.planMode === true,
  };
}
...
function normalizeAppSettings(
  ...
  const analyticsEnabled =
    typeof source?.analyticsEnabled === "boolean"
      ? source.analyticsEnabled
      : true;
  if (
    source?.analyticsEnabled !== undefined &&
    typeof source.analyticsEnabled !== "boolean"
  ) {
  ...
  const rawAnalyticsUserId =
    typeof source?.analyticsUserId === "string"
      ? source.analyticsUserId.trim()
      : "";
  if (
    source?.analyticsUserId !== undefined &&
    typeof source.analyticsUserId !== "string"
  ) {
  ...
  const state: AppSettingsState = {
    ...
    chatSoundPreference: normalizeChatSoundPreference(
      source?.chatSoundPreference,
    ),
    chatSoundId: normalizeChatSoundId(source?.chatSoundId),
    terminal: {
      scrollbackLines: clampNumber(
        source?.terminal?.scrollbackLines,
        DEFAULT_TERMINAL_SCROLLBACK,
        MIN_TERMINAL_SCROLLBACK,
        MAX_TERMINAL_SCROLLBACK,
      ),
      minColumnWidth: clampNumber(
        source?.terminal?.minColumnWidth,
    ...
    editor: {
      preset: editorPreset,
      commandTemplate: normalizeEditorCommandTemplate(
        source?.editor?.commandTemplate,
        editorPreset,
      ),
    },
  ...
  const shouldWrite =
    JSON.stringify(source ? toComparablePayload(source) : null) !==
    JSON.stringify(toFilePayload(state));
  state.warning =
    warnings.length > 0
...
function applyPatch(
  ...
  return normalizeAppSettings(
    ...
    state.filePathDisplay,
  ).payload;
}

export async function readAppSettingsSnapshot(
  filePath = getSettingsFilePath(homedir()),
) {
  try {
...
export class AppSettingsManager {
  ...
  private readonly listeners = new Set<
    (snapshot: AppSettingsSnapshot) => void
  >();
  ...
  async writePatch(patch: AppSettingsPatch) {
    ...
    await writeFile(
      this.filePath,
      `${JSON.stringify(toFilePayload(nextState), null, 2)}\n`,
      "utf8",
    );
    this.setState(nextState);
  ...
  private async readState(options?: { persistNormalized?: boolean }) {
    ...
    try {
      ...
      const normalized = normalizeAppSettings(
        hasText ? JSON.parse(text) : undefined,
        this.filePath,
      );
      if (options?.persistNormalized && (!hasText || normalized.shouldWrite)) {
        await writeFile(
          this.filePath,
          `${JSON.stringify(toFilePayload(normalized.payload), null, 2)}\n`,
          "utf8",
        );
      }
      return {
