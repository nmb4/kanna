...
import { PiAppServerManager, type PiAvailableModel } from "./pi-app-server";
import {
...
export function buildPromptText(
  ...
  return [trimmed || "Please inspect the attached files.", attachmentHint]
    .join("\n\n")
    .trim();
}
...
export function normalizeClaudeUsageSnapshot(
  ...
  return {
    ...
    ...(cacheReadInputTokens > 0
      ? { lastCachedInputTokens: cacheReadInputTokens }
      : {}),
...
export function maxClaudeContextWindowFromModelUsage(
  ...
  for (const value of Object.values(record)) {
    const usage = asRecord(value);
    const contextWindow =
      asNumber(usage?.contextWindow) ?? asNumber(usage?.context_window);
    ...
    return [
      timestamped({
        ...
        slashCommands: Array.isArray(message.slash_commands)
          ? message.slash_commands.filter(
              (entry: string) => !entry.startsWith("._"),
            )
          : [],
      ...
      timestamped({
        ...
        result:
          typeof message.result === "string"
            ? message.result
            : stringFromUnknown(message.result),
        costUsd:
...
async function* createClaudeHarnessStream(
  ...
  for await (const sdkMessage of q as AsyncIterable<any>) {
    const sessionToken =
      typeof sdkMessage.session_id === "string" ? sdkMessage.session_id : null;
    if (sessionToken) {
    ...
    if (sdkMessage?.type === "result") {
      ...
      const finalUsage = latestUsageSnapshot
        ? {
            ...latestUsageSnapshot,
            ...(typeof (resultContextWindow ?? lastKnownContextWindow) ===
            "number"
              ? { maxTokens: resultContextWindow ?? lastKnownContextWindow }
              : {}),
            ...(accumulatedUsage &&
            accumulatedUsage.usedTokens > latestUsageSnapshot.usedTokens
...
export class AgentCoordinator {
  ...
  private readonly startClaudeSessionFn: NonNullable<
    AgentCoordinatorArgs["startClaudeSession"]
  >;
   ...
   * temporary process.  Returns an empty array if Pi is not installed.
   */
  async getPiAvailableModels(cwd?: string): Promise<PiAvailableModel[]> {
    try {
      return await this.piManager.fetchAvailableModels(cwd);
    } catch {
      return [];
    }
  }
  ...
  private getProviderSettings(
    ...
    if (provider === "pi") {
      const modelOptions = normalizePiModelOptions(options.modelOptions);
      return {
        model: normalizeServerModel(provider, options.model),
        effort: modelOptions.thinkingLevel,
        serviceTier: undefined,
        planMode: false,
      };
    }
  ...
  private async dequeueAndStartQueuedMessage(
    ...
    await this.startTurnForChat({
      chatId,
      provider,
      content: options?.steered
        ? buildSteeredMessageContent(queuedMessage.content)
  ...
  private async maybeStartNextQueuedMessage(chatId: string) {
    ...
    const nextQueuedMessage =
      typeof this.store.getQueuedMessages === "function"
  ...
  private async startTurnForChat(args: {
    ...
    const shouldGenerateTitle =
      args.appendUserPrompt &&
      chat.title === "New Chat" &&
      existingMessages.length === 0;
    const optimisticTitle = shouldGenerateTitle
      ? fallbackTitleFromMessage(args.content)
      : null;
    ...
    if (args.provider === "claude") {
      ...
      const sessionToken = await this.piManager.startSession({
        chatId: args.chatId,
    ...
    const active: ActiveTurn = {
      ...
      planMode: args.planMode,
      status:
        args.provider === "claude" || args.provider === "pi"
          ? "running"
          : "starting",
      pendingTool: null,
    ...
    this.emitStateChange(args.chatId, {
      immediate: active.status === "starting",
    });
  ...
  async steer(command: Extract<ClientCommand, { type: "message.steer" }>) {
    const queuedMessage = this.store.getQueuedMessage(
      command.chatId,
  ...
  async dequeue(command: Extract<ClientCommand, { type: "message.dequeue" }>) {
    ...
    await this.store.removeQueuedMessage(
      command.chatId,
      command.queuedMessageId,
    );
  }
  ...
  private async runClaudeSession(session: ClaudeSessionState) {
    try {
      for await (const event of session.session.stream) {
        ...
        const completedClaudePromptSeq =
          event.entry.kind === "result" || event.entry.kind === "interrupted"
  ...
  async respondTool(
    command: Extract<ClientCommand, { type: "chat.respondTool" }>,
  ) {
