...
export interface PiModelOptions {
  thinkingLevel: PiReasoningEffort;
}

/** A model entry returned by Pi's get_available_models RPC command. */
...
export function isPiReasoningEffort(
  value: unknown,
): value is PiReasoningEffort {
...
export function normalizePiModelId(
  ...
  // Pi supports arbitrary model IDs configured by the user, so we pass
  // through anything that looks like a model ID and only fall back to default.
...
export type ChatCheckoutBranchResult =
  | ChatCheckoutBranchSuccess
  | ChatCheckoutBranchFailure;
...
export type ChatCreateBranchResult =
  | ChatCreateBranchSuccess
  | ChatCreateBranchFailure;
...
export type ChatMergeBranchResult =
  | ChatMergeBranchSuccess
  | ChatMergeBranchFailure;
