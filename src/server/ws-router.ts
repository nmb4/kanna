...
interface CreateWsRouterArgs {
  ...
  appSettings?: Pick<AppSettingsManager, "getSnapshot" | "write"> &
    Partial<Pick<AppSettingsManager, "writePatch" | "onChange">>;
...
export function createWsRouter({
  ...
  const resolvedDiffStore = diffStore ?? {
    ...
    checkGitHubRepoAvailability: async () => ({
      available: false,
      message: "Unavailable",
    }),
  ...
  const resolvedAppSettings = {
    ...
    writePatch: async (patch: AppSettingsPatch) => {
      ...
      if (
        ...
        return await appSettings.write({
  ...
  function getProtectedChatIds() {
    ...
    const drainingChatIds =
      typeof agent.getDrainingChatIds === "function"
        ? agent.getDrainingChatIds()
        : new Set<string>();
    return new Set([...activeStatuses.keys(), ...drainingChatIds.values()]);
  }

  function getProtectedDraftChatIds(
    extraSockets?: Iterable<ServerWebSocket<ClientState>>,
  ) {
  ...
  function getSidebarSnapshotCacheEntry(cache?: SnapshotComputationCache) {
    ...
    if (isSendToStartingProfilingEnabled()) {
      const totalChats = data.projectGroups.reduce(
    ...
    return {
      ...
      snapshot: {
        ...
        data: deriveChatSnapshot(
          ...
          topic.chatId,
          (chatId) =>
            store.getRecentChatHistory(
              chatId,
              topic.recentLimit ?? DEFAULT_CHAT_RECENT_LIMIT,
            ),
        ),
      },
    };
  ...
  async function pushSnapshots(
    ...
    for (const [id, topic] of ws.data.subscriptions.entries()) {
      ...
      const signatureReadyAt =
        topic.type === "sidebar" ? createdAt : performance.now();
      ...
      if (
        topic.type === "chat" &&
        envelope.snapshot.type === "chat" &&
        envelope.snapshot.data?.runtime.status === "starting"
      ) {
  ...
  function pushTerminalSnapshot(terminalId: string) {
    for (const ws of sockets) {
      const snapshotSignatures = ensureSnapshotSignatures(ws);
      for (const [id, topic] of ws.data.subscriptions.entries()) {
        if (topic.type !== "terminal" || topic.terminalId !== terminalId)
          continue;
      ...
      switch (command.type) {
        ...
        case "settings.readKeybindings": {
          send(ws, {
            ...
            type: "ack",
            id,
            result: keybindings.getSnapshot(),
          });
          return;
        }
        ...
        case "settings.writeAppSettingsPatch": {
          ...
          send(ws, { v: PROTOCOL_VERSION, type: "ack", id, result: snapshot });
          if (
            command.patch.analyticsEnabled !== undefined &&
            previousAnalyticsEnabled &&
        ...
        case "settings.readLlmProvider": {
          send(ws, {
            ...
            type: "ack",
            id,
            result: await resolvedLlmProvider.read(),
          });
          return;
        }
        ...
        case "settings.getPiModels": {
          const models = await agent.getPiAvailableModels(command.cwd);
        ...
        case "project.open": {
          ...
          const existingProjectId =
            store.state.projectIdsByPath.get(normalizedPath);
        ...
        case "chat.send": {
          ...
          const profile =
            command.clientTraceId && result.chatId
              ? agent.getActiveTurnProfile(result.chatId)
              : null;
          logSendToStartingProfile(
            profile?.traceId ?? command.clientTraceId,
            profile?.startedAt,
            "ws.chat_send_ack",
            {
        ...
        case "chat.refreshDiffs": {
          ...
          const changed = await resolvedDiffStore.refreshSnapshot(
            project.id,
            project.localPath,
          );
          send(ws, { v: PROTOCOL_VERSION, type: "ack", id });
        ...
        case "chat.loadHistory": {
          ...
          if (!chat) throw new Error("Chat not found");
          const page = store.getMessagesPageBefore(
            command.chatId,
