...
const ChatInputInner = forwardRef<ChatInputHandle, Props>(function ChatInput(
  ...
  function setPiThinkingLevel(thinkingLevel: PiReasoningEffort) {
    updateComposerState((state) =>
      state.provider !== "pi"
        ? state
        : { ...state, modelOptions: { thinkingLevel } },
    );
  }
  ...
  const enqueueFiles = useCallback(
    (files: File[]) => {
      ...
      if (
        ...
        setUploadError(
          `You can upload up to ${MAX_FILES_PER_DROP} files at a time.`,
        );
        return;
      }
        ...
        <div className="max-w-[840px] mx-auto rounded-[32px]">
          ...
          <div className="flex items-end max-w-[840px] mx-auto border dark:bg-card/40 backdrop-blur-lg border-border rounded-[29px] pr-1.5">
            ...
            <Button
              ...
              disabled={
                disabled || (!canCancel && !canSubmit) || hasPendingUploads
              }
              size="icon"
      ...
      >
        <div className="overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden flex flex-row">
          ...
          <ChatPreferenceControls
            ...
            piModels={
              selectedProvider === "pi" && piModels.length > 0
                ? piModels
                : undefined
            }
            ...
            onModelChange={(_, model) => {
              if (providerLocked) {
                updateComposerState((state) =>
                  withNormalizedContextWindow(state, model),
                );
                return;
              }
            ...
            onModelOptionChange={(change) => {
              switch (change.type) {
                ...
                case "codexReasoningEffort":
                  setReasoningEffort(change.effort);
                  break;
                case "piThinkingLevel":
                  setPiThinkingLevel(change.level);
                  break;
                case "contextWindow":
