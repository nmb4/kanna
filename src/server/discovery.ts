  ...
  for (const project of projects) {
    ...
    if (!existing || project.modifiedAt > existing.modifiedAt) {
      merged.set(project.localPath, {
        ...
        title:
          project.title ||
          path.basename(project.localPath) ||
          project.localPath,
...
function readCodexSessionMetadata(sessionsDir: string) {
  ...
  for (const sessionFile of collectCodexSessionFiles(sessionsDir)) {
    ...
    const cwd =
      typeof payloadRecord.cwd === "string" ? payloadRecord.cwd : null;
    ...
    const payloadTimestamp =
      typeof payloadRecord.timestamp === "string"
        ...
        : Number.NaN;
    const modifiedAt =
      [recordTimestamp, payloadTimestamp, fileStat.mtimeMs].find(
...
export class PiProjectDiscoveryAdapter implements ProjectDiscoveryAdapter {
  ...
  scan(homeDir: string = homedir()): ProviderDiscoveredProject[] {
    ...
    for (const sessionFile of collectSessionFiles(sessionsDir)) {
      try {
        const fileStat = statSync(sessionFile);
        const firstLine = readFileSync(sessionFile, "utf8").split("\n", 1)[0];
        if (!firstLine?.trim()) continue;
